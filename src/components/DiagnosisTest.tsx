...
export const DiagnosisTest: React.FC = () => {
    ...
    if (selected) {
      const timer = setTimeout(() => {
        if (currentQIndex < QUESTIONS.length - 1) {
          ...
          // Save results (mock)
          localStorage.setItem('diagnosisScore', '85');
          router.push('/results');
        }
      }, 800);
      return () => clearTimeout(timer);
    }
  }, [selected, currentQIndex, router]);
    ...
    <div className="flex min-h-screen flex-col bg-[#FAF8F1] dark:bg-bg-dark p-6 max-w-md mx-auto w-full transition-colors duration-300">
      ...
      <div className="grid grid-cols-2 gap-4 pb-8">
        {question.options.map((opt, idx) => {
          ...
          // const isCorrect = opt === question.correct; // Logic for scoring would go here
